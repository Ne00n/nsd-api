...
def getCert(domains,fullDomain):
    ...
    if cert:
        ...
        with open(f"certs/{fullDomain}-fullchain.pem", 'w') as out:
...
for fullDomain in domains['domains']:
    ...
    if not os.path.isfile(f"certs/{fullDomain}-fullchain.pem") or not os.path.isfile(f"certs/{fullDomain}-privkey.pem"):
        print(f"Certificate not found for {fullDomain}")
        getCert(domains,fullDomain)
    else:
        print(f"Certificate found for {fullDomain}")
        print(f"Checking Certificate age for {fullDomain}")
        if os.path.getmtime(f"certs/{fullDomain}-fullchain.pem") + (86400 * 30) < datetime.now().timestamp():
